...
export const PrimaryLogoSvg: React.FC<{ theme?: 'dark' | 'light' | 'gradient'; size?: number }> = ({
  ...
  const tagBg = isLight ? '#e4e4e7' : '#18181b';
  const tagBorder = isLight ? '#cbd5e1' : '#27272a';
  ...
  return (
    <div className="flex items-center space-x-3.5 select-none">
      ...
      >
        <div className={`w-full h-full rounded-[14px] flex items-center justify-center ${theme === 'gradient' ? 'bg-gradient-to-br from-blue-600 to-indigo-700' : isLight ? 'bg-white' : 'bg-zinc-950'}`}>
          <Sparkles className={`w-1/2 h-1/2 ${theme === 'gradient' ? 'text-white' : isLight ? 'text-blue-600' : 'text-cyan-400'}`} />
        </div>
      </div>
...
export const WordmarkSvg: React.FC<{ theme?: 'dark' | 'light' | 'gradient' }> = ({ theme = 'dark' }) => {
  const isLight = theme === 'light';
  return (
    <div className="flex items-baseline space-x-2 tracking-tight select-none">
...
export const HorizontalLockupSvg: React.FC<{ theme?: 'dark' | 'light' | 'gradient' }> = ({ theme = 'dark' }) => {
  return (
    <div className={`px-6 py-3 rounded-2xl border flex items-center space-x-4 ${theme === 'light' ? 'bg-white border-zinc-200' : theme === 'gradient' ? 'bg-zinc-900/90 border-blue-500/30' : 'bg-zinc-950 border-zinc-800'}`}>
      <LogomarkStandaloneSvg size={40} mode="gradient" />
      <div className="h-8 w-[1px] bg-zinc-700/40" />
      <WordmarkSvg theme={theme} />
    </div>
  );
};
...
export const StackedLockupSvg: React.FC<{ theme?: 'dark' | 'light' | 'gradient' }> = ({ theme = 'dark' }) => {
  const isLight = theme === 'light';
  return (
    <div className={`p-8 rounded-2xl border flex flex-col items-center justify-center space-y-4 text-center ${isLight ? 'bg-white border-zinc-200' : theme === 'gradient' ? 'bg-zinc-900/90 border-blue-500/30' : 'bg-zinc-950 border-zinc-800'}`}>
      <LogomarkStandaloneSvg size={72} mode="gradient" />
...
export const MonochromeLockupSvg: React.FC<{ mode: 'dark-on-light' | 'light-on-dark' }> = ({ mode }) => {
  ...
  return (
    <div className={`p-6 rounded-2xl border flex items-center space-x-4 ${isLightMode ? 'bg-white border-zinc-300 text-zinc-950' : 'bg-zinc-950 border-zinc-800 text-white'}`}>
      <LogomarkStandaloneSvg size={44} mode={isLightMode ? 'mono-light' : 'mono-dark'} />
      <div className="flex flex-col">
        <span className={`text-xl font-bold tracking-tight ${isLightMode ? 'text-zinc-950' : 'text-white'}`}>
          MEGAVAULT
        </span>
        <span className={`text-[10px] font-semibold tracking-widest uppercase ${isLightMode ? 'text-zinc-600' : 'text-zinc-400'}`}>
          MONOCHROME SPEC
        </span>
      </div>
    </div>
